...
The fit takes a few seconds on a laptop, produces a tree in which each of the four classes occupies a coherent subtree, and keeps domain-purity high: approximately 88% of tree edges connect two nodes from the same class, and each of the four classes contributes multiple dense, pure ($>95\%$) subtrees of $\geq 100$ nodes (33 such subtrees in total) ([Figure 1]). The interactive HTML export lets a biologist explore the tree by any of the metadata columns — organism, source proteome, MitoCarta compartment — and follow the unique shortest path between any two proteins.
